	...
	function pingAPI(s, itemNumber, theNewToken, environment, db, data, userInfo){
		...
		for(var k=0; k<dataDump.order_specs.length; k++){
			...
			if(dataDump.order_specs[k].code == "PPR"){
				specs.paper = addToTable(s, db, "specs_paper", dataDump.order_specs[k].value, dataDump.job_item_id, data, userInfo, null, dataDump.order_specs[k], "paper", dataDump);
			}
		...
		// --------------------------------------------------------------------------------
		// Add data to the attributes tables
		// None of these are being returned out of the addToTable() function currently, they are just for logging purposes.
		var attributeMappings = [
			{name: "Material", key: "material", table: "attr_material"},
			{name: "Paper Type", key: "paperType", table: "attr_paper-type"},
			{name: "Paper", key: "attrPaper", table: "attr_paper"},
			{name: "Stock", key: "stock", table: "attr_stock"},
			{name: "Paper Stock", key: "paperStock", table: "attr_paper-stock"},
			{name: "Substrate", key: "paperStock", table: "attr_substrate"}
		];

		// Loop throughh the attributeMapping array and post them to the tables.
		for (var k = 0; k < dataDump.display_specs.length; k++) {
			var spec = dataDump.display_specs[k];
			for (var j = 0; j < attributeMappings.length; j++) {
				var map = attributeMappings[j];
				if (spec.attribute_name == map.name) {
					specs[map.key] = addToTable(s, db, map.table, spec.attr_value, dataDump.job_item_id, data, userInfo, null, spec, "attr");
				}
			}
		}
